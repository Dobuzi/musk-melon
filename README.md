...
이 앱은 테슬라, 스페이스X, xAI, 뉴럴링크, 더 보링 컴퍼니, x 또는 일론 머스크와 무관합니다. 소속·후원·제휴를 주장하지 않으며, 공식 상표·로고를 사용하지 않습니다. 원문 에세이·보도자료를 그대로 옮기지 않고 공개 사실만 요약합니다.
...
- UI는 src/lib/social.ts의 SocialStore 인터페이스만 바라봅니다. 이후 Supabase 같은 백엔드로 교체해도 화면을 다시 쓰지 않아도 됩니다.
